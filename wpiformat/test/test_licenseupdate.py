...
"""
        ...
        run_and_check_file(LicenseUpdate(), test_hpp, contents, contents, True)
...
"""
    ...
    """
        run_and_check_file(LicenseUpdate(), excluded_hpp, contents, contents, True)

        # Ensure excluded files won't be processed
        config_file = Config(Path.cwd(), Path(".wpiformat"))
        assert not LicenseUpdate().should_process_file(config_file, excluded_hpp)
